...
| `SERVE_NL2SQL_GRAPH_EXPAND` | No | unset (**enabled**) | `0`/`false`/`off`/`no` turns OFF, and `1`/`true`/`on`/`yes` pins ON, the appending of the tables one induced foreign-key hop from the *retrieved* classes — with their columns — to the flat (non-deep-reasoning) NL→SQL schema context ([`OntologyGraphTool.expand_from`](src/coa_serve/tier2/tools/graph_tools.py)). Requires a graph client (Neptune) to be wired; a deployment without one skips the walk and generates from retrieval alone rather than erroring. This is the same FK walk `SERVE_DEEP_REASONING_GRAPH_TRAVERSAL` offers the agent as a tool, but here it fires on every generation: the flat path retrieves **once** — its 2-shot `correct()` re-generates on an execution error or an empty result, but never re-retrieves — so a table similarity did not rank has no other route into the prompt. The walked tables are labelled as **not** similarity-matched and annotated with the join key, and are appended after — never in place of — the retrieved block, whose text carries sampled values the ontology cannot supply; the writer decides whether to use them. Per-request override: `options.flatGraphExpand`, which pins either direction and is what lets on/off run paired against one image — with the walk on by default, an A/B's control arm must pass `flatGraphExpand: false` rather than omit the option. An unrecognised value at either layer defers to the next one instead of meaning "off". **On by default** on the evidence of three repeats each of three cells: retrieval full-hit up in 8 of the 9 repeats, **+6 to +20.8pp**, and execution accuracy up in **6 of 6** Spider 2.0 arm×seed contrasts, **+2.2 to +7.4pp** (810 paired questions, 67 win / 35 loss, exact p=0.002; question-clustered paired bootstrap +3.95pp, CI [+1.11, +7.04] excluding zero), at unchanged latency. The honest caveats: BIRD's execution accuracy is a three-repeat wash (+0.2pp, 322 win / 314 loss, p=0.78) despite +11.9pp full-hit, and collapsing Spider's repeats to a per-question majority leaves 32 win / 21 loss, p=0.17 — so the claim is "a real gain on Spider 2.0, neutral on BIRD", not "a gain everywhere". |
| `SERVE_NL2SQL_GRAPH_EXPAND_MAX_TABLES` | No | `8` (min 0) | How many walked tables `SERVE_NL2SQL_GRAPH_EXPAND` may append to the flat NL→SQL schema context. A budget, not a tuned optimum: the walk is unconditional and ordered by FK hop then label — adjacency, not question similarity — so on a wide schema the hop-1 frontier is dozens of tables and an uncapped appended block would outweigh the retrieved one it is meant to supplement. 8 is both roughly the 7 tables retrieval itself contributes (`DEFAULT_RETRIEVAL_K`) and the value every cell behind the default-on decision was measured at — where it bound on 22.3% of walked questions, so a larger default would put unmeasured behaviour on about a fifth of traffic. Raise it when the missing join table is plausibly further down the FK ordering and the prompt has room; lower it (or set `0`, which appends nothing) to isolate the rest of the flag's behaviour. Non-numeric values fall back to the default and log `nl_to_sql_graph_expand_max_tables_invalid`; the effective cap and whether it bound are on every `nl_to_sql_graph_expand` line (`max_tables`, `capped`). The graph tool's own `MAX_NODE_LIMIT` (200) is the hard ceiling. |
